...
export function BrandsPanel({ contentEntrance }: { contentEntrance: string }) {
    return (
        <div className={cn("mx-auto grid max-w-[1400px] grid-cols-[1fr_1fr_1fr_1fr_220px] gap-4 px-8 py-10 lg:px-12", contentEntrance)}>
            {BRANDS.map((brand) => (
                <NavigationMenuLink key={brand.name} asChild>
                    <Link
                        href={`/products?brand=${brand.name.toLowerCase().replace(/\s+/g, "-")}`}
                        className="group relative flex min-h-[260px] flex-col justify-between overflow-hidden bg-charcoal text-white transition-shadow duration-300 hover:shadow-sm"
                    >
                        ...
                        <div className="relative z-10 p-7">
                            ...
                            <h4 className="mt-1.5 text-[1rem] font-bold leading-snug">
                                {brand.name}
                            </h4>
                        ...
                        <div className="relative z-10 p-7 pt-0">
                            ...
                            <span className="inline-flex items-center gap-1.5 text-[12px] font-bold text-white/50 transition-colors duration-300 group-hover:text-white">
                                Explore Brand
            ...
            <div className="flex flex-col justify-between border-l border-border-warm/30 pl-4">
                <div>
                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-text-muted/50">
