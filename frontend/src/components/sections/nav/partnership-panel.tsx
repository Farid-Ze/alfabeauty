...
export function PartnershipPanel({ contentEntrance }: { contentEntrance: string }) {
    return (
        <div className={cn("mx-auto grid max-w-[1400px] grid-cols-[1fr_1fr_1fr_220px] gap-px bg-border-warm/20 px-8 py-10 lg:px-12", contentEntrance)}>
            {PARTNERSHIP_ITEMS.map((item) => (
                <div key={item.title}>
                    <NavigationMenuLink asChild>
                        <Link
                            href={item.href}
                            className="group relative flex min-h-[200px] flex-col justify-between overflow-hidden bg-background p-7 transition-colors duration-300 hover:bg-muted/40"
                        >
                            <div>
                                <div className="flex h-11 w-11 items-center justify-center bg-muted transition-colors duration-300 group-hover:bg-brand-crimson/10">
                                    <item.icon className="h-5 w-5 text-muted-foreground transition-colors duration-300 group-hover:text-brand-crimson" />
                                </div>
                                <h4 className="mt-5 text-[14px] font-bold leading-snug">{item.title}</h4>
                            ...
                            <div className="mt-5">
                                ...
                                <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-foreground/50 transition-colors duration-300 group-hover:text-foreground">
                                    Learn More
                                    <ArrowRight className="h-3.5 w-3.5 transition-transform duration-300 group-hover:translate-x-0.5" />
                                </span>
                            </div>

                            <div className="absolute bottom-0 left-0 h-0.5 w-0 bg-brand-crimson transition-all duration-500 group-hover:w-full" />
                        </Link>
                    </NavigationMenuLink>
                </div>
            ))}
